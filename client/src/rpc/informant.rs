...
//             i => i as usize + 1,
//         };
//         let sum: usize = self.samples[0..now].iter().map(|x| *x as
// usize).sum();         sum / now
//     }
...
// impl<T: fmt::Display + Default + Copy + Ord> fmt::Debug for
// StatsCalculator<T> {     fn fmt(&self, fmt: &mut fmt::Formatter) ->
// fmt::Result {         write!(fmt, "median: {} ms",
// self.approximated_median())     }
// }
