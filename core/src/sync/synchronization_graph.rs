...
impl SynchronizationGraph {
    ...
    ) -> Self
    {
        ...
        let consensus_weak = Arc::downgrade(&consensus);
        // It receives `BLOCK_GRAPH_READY` blocks in order and handles them in
        // `ConsensusGraph`
        thread::Builder::new()
            .name("Consensus Worker".into())
            .spawn(move || loop {
                // try receive first, then see if consensus is available
                match (
                    consensus_receiver.recv_blocking(),
                    consensus_weak.upgrade(),
                ) {
                    (Some((hash, ignore_body)), Some(consensus)) => {
