...
impl RequestManager {
    ...
    pub fn request_headers_unchecked(
        &self, io: &NetworkContext, peer_id: PeerId, hashes: Vec<H256>,
    ) {
        if let Err(e) = self.request_handler.send_request(
            io,
    ...
    /// Remove in-flight epochs, and epochs requested before will be delayed.
    /// If `peer_id` is `None`, all epochs will be delayed and `hashes` will
    /// always become empty.
    ...
    pub fn request_epochs_unchecked(
